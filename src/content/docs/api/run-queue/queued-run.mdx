...
| --- | --- | --- |
| `queue_ref` | `string` | The queue item ID. Use it to [activate](/api/run-queue/activate-items/) or [release](/api/run-queue/release-items/) the run later. When calling the activate endpoint, pass an array of these values as `queue_refs` (e.g. `{"queue_refs": ["123456"]})`. |
